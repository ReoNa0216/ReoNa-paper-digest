...
- 工具：`cover-gen.py`（ZenMux `gpt-image-2`，OpenAI 兼容端点，1280×544 → 900×383）；密钥走 `secrets_env.py`（环境变量 `ZENMUX_API_KEY` 或工作区根 `.env`）。
...
| `cover-gen.py` | ZenMux 生成封面 | 1280×544 → 900×383；密钥走 `secrets_env.py` |
